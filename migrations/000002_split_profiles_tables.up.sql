...
-- On a fresh bootstrap, profiles table does not exist; data migration is skipped.
...
-- Data migration from legacy profiles table (only runs if profiles table exists)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
        ...
        SELECT
            ...
            description,
            age,
            COALESCE(height_cm, height),
            COALESCE(weight_kg, weight),
            gender,
            clothing_size,
            shoe_size,
            COALESCE(experience_years, experience),
            hourly_rate,
            city,
            country,
            ...
            COALESCE(barter_accepted, FALSE),
            COALESCE(accept_remote_work, FALSE),
            COALESCE(travel_cities, '[]'::jsonb),
            COALESCE(visibility, 'public'),
            COALESCE(view_count, profile_views, 0),
            COALESCE(rating, 0),
        ...
        -- Drop the legacy profiles table after migration
        DROP TABLE profiles;
    END IF;
END $$;